...
/**
 * What this person last did on each exercise of a training day.
 *
 * The set logger used to pre-fill kg and reps from the coach's prescription
 * alone, so someone who works at 60 kg on a row the coach wrote as 50 retyped
 * the same number every session, and a solo program with no target opened
 * empty every time. This is the memory that fixes it: the most recent set the
 * person logged for each exercise, keyed by `program_exercises.id` and again
 * by exercise name, so a set logged before the program link existed still
 * counts.
 *
 * One query, capped: a day is a handful of lifts and the newest rows are all
 * that matter, so the pool is the last 400 sets rather than a per-exercise
 * round trip each.
 */
export type LastPerformance = { weight_kg: number; reps: number; at: string };

export async function getLastPerformance(day: ClientWorkoutDay): Promise<Record<string, LastPerformance>> {
  const live = await liveUser();
  if (!live) return {};
  const ids = day.exercises.map((e) => e.exercise_id).filter((id): id is string => Boolean(id));
  if (ids.length === 0) return {};
  const { supabase, userId } = live;
  const { data, error } = await supabase
    .from("logged_sets")
    .select("program_exercise_id, exercise_id, weight_kg, reps, received_at, exercise:exercises(name_en, name_ro)")
    .eq("user_id", userId)
    .in("exercise_id", ids)
    .gt("reps", 0)
    .order("received_at", { ascending: false })
  ...
  if (error) return {};
  type Row = {
    program_exercise_id: string | null; exercise_id: string;
    weight_kg: number | null; reps: number | null; received_at: string;
    exercise: { name_en: string; name_ro: string | null } | null;
  };
  const out: Record<string, LastPerformance> = {};
  // Rows arrive newest first, so the first one to claim a key is the answer.
  for (const row of (data ?? []) as unknown as Row[]) {
    const entry: LastPerformance = { weight_kg: row.weight_kg ?? 0, reps: row.reps ?? 0, at: row.received_at };
    const name = row.exercise?.name_ro ?? row.exercise?.name_en ?? null;
    for (const key of [row.program_exercise_id, row.exercise_id, name]) {
      if (key && !(key in out)) out[key] = entry;
    }
  }
  return out;
}
