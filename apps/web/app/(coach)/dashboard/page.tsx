...
export default async function DashboardPage() {
  ...
  return (
    <div>
      <PageTitle title="Who needs you today?" />
      <div className="mb-5 flex flex-wrap gap-3">
        <StatCard label="Active clients" value={rows.length} />
        <StatCard label="Check-ins to review" value={checkIns.length} accent={checkIns.length > 0} />
        <StatCard label="Unread messages" value={unread} accent={unread > 0} />
        <StatCard label="At risk" value={atRisk} accent={atRisk > 0} />
      </div>

      {rows.length === 0 ? (
        <EmptyState title="No clients yet" hint="Invite your first client from the Clients page." />
      ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.client_id} className="border-b border-line last:border-0">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 text-ink-soft">{timeAgo(r.last_activity)}</td>
                  <td className="px-4 py-3 text-right">
                    {r.pending_checkin ? (
                      <Link href="/check-ins" className="font-semibold text-accent-ink hover:underline">Review</Link>
                    ) : (
                      <Link href={`/clients?focus=${r.client_id}`} className="font-semibold text-accent-ink hover:underline">Open</Link>
                    )}
                  </td>
                </tr>
              ))}
