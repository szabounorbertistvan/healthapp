...
import { groupHabitsByCategory, suggestionKeyForName } from "@/lib/habit-suggestions";
 ...
 *
 * The rows come grouped by category (nutrition, movement, recovery, then the
 * person's own) under a small label each — but only once there are two or
 * more groups: a lone "Your own" heading over a list of custom habits says
 * nothing.
 *
 * Two contexts, one component: on the Habits page (`removable`) it is the full
 * list, with hairlines between the rows; inside the Today habits card it sits
 * in a tight indented column, so the row stays compact and carries no gutter
 * of its own.
 */
export function HabitTicks({ habits, removable = false }: { habits: ClientHabitRow[]; removable?: boolean }) {
  ...
  // Grouped from the optimistic rows, so a tick never jumps a habit around.
  const groups = groupHabitsByCategory(rows);
  const labelled = groups.length > 1;

  return (
    <div className={`${removable ? "space-y-5" : "space-y-3"} ${pending ? "opacity-70" : ""}`}>
      {groups.map((group) => (
        <section key={group.category}>
          {labelled ? (
            <p className={`mb-1 text-[10px] font-semibold uppercase tracking-wider text-ink-faint ${removable ? "" : "px-1.5"}`}>
              {t.clientWidgets.habitCategories[group.category]}
            </p>
          ) : null}
          <ul className={removable ? "divide-y divide-line/60" : "space-y-0.5"}>
